...
public class CPack extends OPack {
    ...
    public CPack(int id, String nombre, String costo, int stock, boolean estado, List<OPackDetalle> detallearticulos) {
        super(id, nombre, costo, stock, estado, detallearticulos);
    }
        ...
        if (Error.isConfirma()) {
            try {
                ...
                if(rs.next())
                {
                    ...
                    for (OPackDetalle packdeta : this.getArticulos()) {
                        Preparando2 = Sql.getCon().prepareStatement("INSERT INTO Pack_has_articulo(PCK_ID_PACK, ART_ID_ARTICULO, CANTIDAD) VALUES(?,?,?)");
                        Preparando2.setInt(1, last_inserted_id);
                        Preparando2.setInt(2, packdeta.getArticulo().getId());
                        Preparando2.setInt(3, packdeta.getCantidad());
                        Preparando2.execute();
                    }
