...
public List<OArticulos> Listar(){
 ...
 public List<OCatArticulos> ListarActivos(String Tipo) {        
     ...
     public List<OArticulos> Buscar(int datoBusqueda, String desde , String hasta){
        ...
        if(Error.isConfirma()){
            ...
            try {
                //Preparando = Sql.getCon().prepareStatement("SELECT * FROM Articulo A inner join Categoria C on A.CAT_ID_CATEGORIA = C.CAT_ID_CATEGORIA" );
                if(datoBusqueda==0){
                    Preparando = Sql.getCon().prepareStatement("SELECT * FROM Articulo A inner join Categoria C on A.CAT_ID_CATEGORIA = C.CAT_ID_CATEGORIA WHERE (ART_FECHA_VENCIMIENTO >= CAST('"+ desde + "'AS DATE) AND ART_FECHA_VENCIMIENTO <= CAST('"+ hasta +"'AS DATE))");

                }else{
                    Preparando = Sql.getCon().prepareStatement("SELECT * FROM Articulo A inner join Categoria C on (A.CAT_ID_CATEGORIA = "+datoBusqueda+" AND C.CAT_ID_CATEGORIA ="+datoBusqueda+")WHERE (ART_FECHA_VENCIMIENTO >= CAST('"+ desde + "'AS DATE) AND ART_FECHA_VENCIMIENTO <= CAST('"+ hasta +"'AS DATE))");
